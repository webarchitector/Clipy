...
public extension HotKeyCenter {
    /// Snapshot of currently registered hotkeys. Used by clients that need to
    /// re-dispatch matching events from a higher-priority source (e.g. a
    /// CGEventTap installed to bypass apps that consume events before the
    /// Carbon hotkey dispatcher sees them).
    var registeredHotKeys: [HotKey] {
        return Array(hotKeys.values)
    }
}
...
public extension HotKeyCenter {
    ...
    func register(with hotKey: HotKey) -> Bool {
        ...
        hotKey.registeredKeyCode = hotKey.keyCombo.currentKeyCode
        hotKeyCount += 1
